...
deepmemetic100000<-function(N,k,P_1,G_1,P_2,G_2,Cruz,BusL,Sub,Com,Me,Mb,alpha,EvaM,salida,nombresalida){
  #print(paste("P_1 ",P_1,";P_2 ",P_2,";G_2 ",G_2, ";Cruz ",Cruz,";Com ",Com))
  evavshv<-paste(nombresalida,"evavshv.txt",sep="_")
  solporgeneracion<-paste(nombresalida,"solporgeneracion.txt",sep="_")
  Eva<<-0
  if(salida==2){
  ...
  while(Eva<EvaM){
    ...
    #inicia la capa 2
    if(P_2>0 & G_2>0 & Medoids[2,k+3]==1 & Eva<EvaM & Eva>(4000)){
    ...
    write.table(as.matrix(t(c(Eva,hiperVolumen(N,k,Medoids)))),file=evavshv,append=TRUE,quote=FALSE,row.names=FALSE,col.names=FALSE)
    datosmatriz<-cbind(Medoids,generacionnivel1)
    write.table(datosmatriz,file=solporgeneracion,append=TRUE,quote=FALSE,row.names=FALSE,col.names=FALSE)
    if(salida==2){
      #print("llegue acá")
